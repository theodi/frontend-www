...
class RootControllerTest < ActionController::TestCase
  ...
  test "courses should have an atom feed" do
    ...
    test "should get previous events page with old events" do
      stub_request(:get, "http://contentapi.dev/with_tag.json?include_children=1&page=1&summary=true&sort=date&role=odi&tag=event").
        to_return(:status => 200, :body => load_fixture('events.json'), :headers => {})
      stub_request(:get, "http://contentapi.dev/with_tag.json?include_children=1&page=1&summary=true&sort=date&tag=course_instance&role=odi").
    ...
    test "should get featured events on events page" do
      ...
      stub_request(:get, "http://contentapi.dev/with_tag.json?include_children=1&page=1&summary=true&sort=date&tag=event&role=odi").
        to_return(:status => 200, :body => load_fixture('events-with-featured.json'), :headers => {})
